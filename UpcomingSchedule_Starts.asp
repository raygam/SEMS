...
response.write "<h1>&nbsp;&nbsp;Upcoming Schedule (Start Date)</h1>"	
response.write "<br>"
...
for i = 0 to 11
	...
	'strSQL = " SELECT * FROM Studies WHERE EstimatedDraftReportDate "
	strSQL = " SELECT * FROM Studies WHERE QuotedExperimentalStartDate "
	strSQL = strSQL & " BETWEEN '" & MMDDYYYY(weekStart) & "' AND '" & MMDDYYYY(weekEnd) & "'"
	strSQL = strSQL & " AND QuotedExperimentalStartDate IS NOT NULL "
	strSQL = strSQL & " ORDER BY QuotedExperimentalStartDate, StudyId"
	...
	response.write "</TR>"
      ...
      if rs.eof=false or rs.bof=false then
		do until rs.eof=true
			...
			If IsBlank(rs("QuotedExperimentalStartDate")) = True Then
				Response.write "<td>&nbsp;</td>"
			Else
				response.write "<TD align=center>" & FlipDateView(Trim(rs("QuotedExperimentalStartDate"))) & "</TD>"
			End If
			If IsBlank(rs("ActualExperimentalStartDate")) = True Then
				Response.write "<td>&nbsp;</td>"
			Else
		...
		response.write "<tr><td></td><td colspan=7>N/A</td></tr>"
	end if
