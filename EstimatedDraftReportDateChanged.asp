...
<%
	Response.Write "<h1>Expected Audited Draft to Client Date Changed</h1>"	
	Response.Write "<p>All studies where the <b>Expected Audited Draft to Client</b> field has been changed in the last 7 days.</p>"
	...
	Response.Write "<form method=post action=EstimatedDraftReportDateChanged.asp>"
	...
	d = dateadd("d",-7,now())
	strsql = "SELECT * FROM Studies WHERE EstimatedDraftReportDate <= '" & formatdate(d) & "' AND ActualDraftReportDate IS NULL"	
	...
	DisplayFields = "Study Number;Financial Client Name;Client Name;Test Substance Name;Study Description;Study Director;Department;OA Status;Change Date;Changed From;Changed To;Changed By"

'Response.Write strsql
	set rsAudit = server.CreateObject("ADODB.Recordset")
	rsAudit.open "SELECT * FROM ChangeLog WHERE ChangeField = 'EstimatedDraftReportDate' AND DateStamp >= '" & formatdate(d) & "' ORDER BY RecordID Asc",db
		...
		if rs.eof=false or rs.bof=false then
			...
			for i = 0 to 7
				...
				if f(i)<>"DaysUntilQDRD" then
					...
					Response.Write "<td align=right valign=top>"
					if isDate(rs("EstimatedDraftReportDate"))=true then
						
						d = cint(datediff("d",now(),rs("EstimatedDraftReportDate")))
			...
			Response.Write "<td align=right valign=top>" & Formatdate(rsAudit("OldValue")) & "</td>"
			Response.Write "<td align=right valign=top>" & Formatdate(rsAudit("NewValue")) & "</td>"
			Response.Write "<td align=right valign=top>" & DBLookup("Users", "UserID", rsAudit("ChangedBy"),"Firstname") & " " & DBLookup("Users", "UserID", rsAudit("ChangedBy"),"Surname")& "</td>"
