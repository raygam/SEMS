...
SELECT CASE rpttype
	CASE 1
		strHeader = "On-Time Starts (By Department) "
	CASE ELSE
...
IF LEN(month) > 0 THEN
	strSQL = strSQL & " AND MONTH(QuotedExperimentalStartDate) = " & month & " " 
END IF

IF LEN(year) > 0 THEN
	strSQL = strSQL & " AND YEAR(QuotedExperimentalStartDate) = " & year & " " 
END IF
	
IF LEN(qtr_months) > 0 THEN
	strSQL = strSQL & " AND MONTH(QuotedExperimentalStartDate) IN  (" & qtr_months & ") " 
END IF
...
strSQL = strSQL & " ORDER BY QuotedExperimentalStartDate, ActualExperimentalStartDate "
...
response.Write "<td width=100 align=center><B>Quoted Experimental Start Date</B></td>"
response.Write "<td width=100 align=center><B>Actual Experimental Start Date</B></td>"
response.Write "<td width=100 align=center><B>OTS Result</B></td>"
response.Write "</tr>"
...
DO UNTIL rs.eof=TRUE
	...
	Response.Write "<TD align=center><a href=""EditStudies.asp?Key=" & rs("WebKey") & """>" & Trim(rs("StudyNumber")) & "</a></TD>"
      ...
      Response.Write "<TD align=center>" & FlipDateView(Trim(rs("QuotedExperimentalStartDate"))) & "</TD>"
      Response.Write "<TD align=center>" & FlipDateView(Trim(rs("ActualExperimentalStartDate"))) & "</TD>"

	' ***  CALCULATE RESULT
	result = "NO"
	IF rs("QuotedExperimentalStartDate") > Date THEN
		result = "N/A"
	END IF
	'IF TRIM(rs("Department")) = "E Fate" THEN
	'	IF IsBlank(rs("ActualExperimentalStartDate")) = False THEN
	'		IF rs("ActualExperimentalStartDate") <= rs("QuotedExperimentalStartDate") THEN
	'			result = "YES"
	'		END IF
	'	END IF
	'ELSE
