...
response.write "<h1>&nbsp;&nbsp;Upcoming Schedule  (Draft Date)</h1>"	
response.write "<br>"
...
for i = 0 to 11
	...
	strSQL = " SELECT * FROM Studies WHERE QuotedDraftReportDate "
	strSQL = strSQL & " BETWEEN '" & MMDDYYYY(weekStart) & "' AND '" & MMDDYYYY(weekEnd) & "'"
	strSQL = strSQL & " AND ActualDraftReportDate IS NULL "
	strSQL = strSQL & " AND QuotedDraftReportDate IS NOT NULL "
	strSQL = strSQL & " ORDER BY QuotedDraftReportDate, EstimatedDraftReportDate, StudyId"
	...
	response.write "</TR>"
      ...
      if rs.eof=false or rs.bof=false then
		do until rs.eof=true
			...
			If IsBlank(rs("QuotedDraftReportDate")) = True Then
				Response.write "<td>&nbsp;</td>"
			Else
				response.write "<TD align=center>" & FlipDateView(Trim(rs("QuotedDraftReportDate"))) & "</TD>"
			End If
			If IsBlank(rs("EstimatedDraftReportDate")) = True Then
				Response.write "<td>&nbsp;</td>"
			Else
	                  response.write "<TD align=center>" & FlipDateView(Trim(rs("EstimatedDraftReportDate"))) & "</TD>"
			End If
			If IsBlank(rs("ExpectedUnauditedDraftToClient")) = True Then
				Response.write "<td>&nbsp;</td>"
			Else
				response.write "<TD align=center>" & FlipDateView(Trim(rs("ExpectedUnauditedDraftToClient"))) & "</TD>"
			End If
                  response.write "</TR>"
