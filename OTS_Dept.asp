...
response.write "<h1>&nbsp;&nbsp;On-Time Starts (By Department) - 2017</h1>"	
response.write "<br>"
response.write "<br>"

response.Write "<form method=post action=OTS_Dept.asp>"
...
FOR intMonth = 1 TO 12
	...
	strSQL = "SELECT * FROM Studies " &_
				" WHERE " &_
				" MONTH(QuotedExperimentalStartDate) = " & intMonth & " " &_
				" AND " &_
				" YEAR(QuotedExperimentalStartDate) = " & intYear & " " &_
	...
	DO UNTIL rs.eof=TRUE
		...
		IF intMonth <= MONTH(NOW()) THEN
			...
			' ***  SEE IF DEADLINE WAS MET.  
			'IF TRIM(rs("Department")) = "E Fate" THEN
			'		IF IsBlank(rs("ActualUnauditedDraftToClient")) = False THEN
			'		IF MMDDYYYY(rs("ActualUnauditedDraftToClient")) <= _
			'				MMDDYYYY(rs("QuotedDraftReportDate")) THEN
			'			fate_comp = fate_comp + 1
			'		END IF
			'	END IF
			'ELSE
			
			IF IsBlank(rs("ActualExperimentalStartDate")) = False THEN
				IF cdate(MMDDYYYY(rs("ActualExperimentalStartDate"))) <= cdate(MMDDYYYY(rs("QuotedExperimentalStartDate"))) THEN
	...
	response.Write "<td align=center>" & chem_comp & "/" & chem_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=1&month="& intMonth & "&year=" & intYear & "&qtr=>"  & DisplayPct(chem_comp, chem_total) & "</a></td>"
	response.Write "<td align=center>" & eco_comp & "/" & eco_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=2&month="& intMonth & "&year=" & intYear & "&qtr=>" & DisplayPct(eco_comp, eco_total) & "</a></td>"
	response.Write "<td align=center>" & fate_comp & "/" & fate_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=3&month="& intMonth & "&year=" & intYear & "&qtr=>" & DisplayPct(fate_comp, fate_total) & "</a></td>"
	'response.Write "<td align=center>" & pd_comp & "/" & pd_total & "&nbsp;&nbsp;&nbsp;" & DisplayPct(pd_comp, pd_total) & "</td>"
	response.Write "<td align=center>" & reg_comp & "/" & reg_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=4&month="& intMonth & "&year=" & intYear & "&qtr=>" & DisplayPct(reg_comp, reg_total) & "</a></td>"
	response.Write "<td align=center><B>" & (chem_comp + eco_comp + fate_comp + reg_comp) & "/" & (chem_total + eco_total + fate_total + reg_total) & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=&month="& intMonth & "&year=" & intYear & "&qtr=>" & DisplayPct(chem_comp + eco_comp + fate_comp + reg_comp, chem_total + eco_total + fate_total + reg_total) & "</a></B></td>"
	response.Write "</tr>"
	...
	IF intMonth = 3 OR intMonth = 6 OR intMonth = 9 OR intMonth = 12 THEN
		...
		response.Write "<td align=center>" & chem_qtr_comp & "/" & chem_qtr_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=1&month=&year=" & intYear & "&qtr=" & CINT(intMonth)/3 &">" & DisplayPct(chem_qtr_comp, chem_qtr_total) & "</a></td>"
		response.Write "<td align=center>" & eco_qtr_comp & "/" & eco_qtr_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=2&month=&year=" & intYear & "&qtr=" & CINT(intMonth)/3 &">" & DisplayPct(eco_qtr_comp, eco_qtr_total) & "</a></td>"
		response.Write "<td align=center>" & fate_qtr_comp & "/" & fate_qtr_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=3&month=&year=" & intYear & "&qtr=" & CINT(intMonth)/3 &">" & DisplayPct(fate_qtr_comp, fate_qtr_total) & "</a></td>"
		'response.Write "<td align=center>" & pd_qtr_comp & "/" & pd_qtr_total & "&nbsp;&nbsp;&nbsp;" & DisplayPct(pd_qtr_comp, pd_qtr_total) & "</td>"
		response.Write "<td align=center>" & reg_qtr_comp & "/" & reg_qtr_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=4&month=&year=" & intYear & "&qtr=" & CINT(intMonth)/3 &">" & DisplayPct(reg_qtr_comp, reg_qtr_total) & "</a></td>"
		response.Write "<td align=center><B>" & (chem_qtr_comp + eco_qtr_comp + fate_qtr_comp + reg_qtr_comp) & "/" & (chem_qtr_total + eco_qtr_total + fate_qtr_total + reg_qtr_total) & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=&month=&year=" & intYear & "&qtr=" & CINT(intMonth)/3 &">" & DisplayPct(chem_qtr_comp + eco_qtr_comp + fate_qtr_comp + reg_qtr_comp, chem_qtr_total + eco_qtr_total + fate_qtr_total + reg_qtr_total) & "</a></B></td>"
		response.Write "</tr>"
		response.Write "<tr><td colspan=6>&nbsp;</td></tr>"
...
response.Write "<td align=center><B>" & chem_ytd_comp & "/" & chem_ytd_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=1&month=&year=" & intYear & "&qtr=>" & DisplayPct(chem_ytd_comp, chem_ytd_total) & "</a></B></td>"
response.Write "<td align=center><B>" & eco_ytd_comp & "/" & eco_ytd_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=2&month=&year=" & intYear & "&qtr=>" & DisplayPct(eco_ytd_comp, eco_ytd_total) & "</a></B></td>"
response.Write "<td align=center><B>" & fate_ytd_comp & "/" & fate_ytd_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=3&month=&year=" & intYear & "&qtr=>" & DisplayPct(fate_ytd_comp, fate_ytd_total) & "</a></B></td>"
'response.Write "<td align=center><B>" & pd_ytd_comp & "/" & pd_ytd_total & "&nbsp;&nbsp;&nbsp;" & DisplayPct(pd_ytd_comp, pd_ytd_total) & "</B></td>"
response.Write "<td align=center><B>" & reg_ytd_comp & "/" & reg_ytd_total & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=4&month=&year=" & intYear & "&qtr=>" & DisplayPct(reg_ytd_comp, reg_ytd_total) & "</a></B></td>"
response.Write "<td align=center><B>" & (chem_ytd_comp + eco_ytd_comp + fate_ytd_comp + reg_ytd_comp) & "/" & (chem_ytd_total + eco_ytd_total + fate_ytd_total + reg_ytd_total) & "&nbsp;&nbsp;&nbsp;<a href=OTS_StudyList.asp?type=1&dept=&month=&year=" & intYear & "&qtr=>" & DisplayPct(chem_ytd_comp + eco_ytd_comp + fate_ytd_comp + reg_ytd_comp, chem_ytd_total + eco_ytd_total + fate_ytd_total + reg_ytd_total) & "</a></B></td>"
response.Write "</tr>"
