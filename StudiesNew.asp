...
<%
	...
	if Request.Form("ClientEdit")="" and Request.querystring("Clientedit")="" then
		strsql = "SELECT * FROM Studies WHERE OAStatus = 'L' AND ArchiveActDate IS NULL AND StudyDirector = '" & trim(session("LoggedInName")) & "'"
		rs.open strsql,db
...
<blockquote>
<form method="post" action="studiesnew.asp" id=form1 name=form1><table cellspacing="0" cellpadding="5"><tr><td valign="top"><h1>Studies</h1>&nbsp; &nbsp;</td><td valign=top></td>
...
if Request.Form("ClientEdit")="" then
	...
	if MyStudies > "" then
		strsql = "SELECT * FROM Studies WHERE StudyDirector = '" & trim(session("LoggedInName")) & "' AND "
	else
	...
	DisplayFields = "StudyNumberNew;ClientName;TestSubstanceName;StudyDescription;StudyType;StudyDirector;Department;OAStatus;RegulatoryStatus;TotalStudyCost;ProtocolIssuedDate;ActualExperimentalStartDate;ActualExperimentalEndDate;DateStamp"
	...
	if rs.eof=false or rs.bof=false then
		...
		Response.Write "<td><b>Regulatory Status</b></td>"
		...
		Response.Write "<td><b>DateStamp</b></td>"
