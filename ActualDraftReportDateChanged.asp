...
<%
	Response.Write "<h1>Actual Audited Draft to Client Date Changed</h1>"	
	Response.Write "<p>All studies where the <b>Actual Audited Draft to Client</b> field has been changed in the last 7 days.</p>"
	...
	Response.Write "<form method=post action=ActualDraftReportDateChanged.asp>"
...
'Response.Write strsql
	...
	rsAudit.open "SELECT * FROM ChangeLog WHERE ChangeTable = 'Studies' AND ChangeField = 'ActualDraftReportDate' AND DateStamp >= '" & formatdate(d) & "' ORDER BY RecordID Asc",db
	...
	do until rsAudit.eof=true
		...
		if rs.eof=false or rs.bof=false then
			...
			if isdate(rsAudit("OldValue"))=true then
			...
			if isdate(rsAudit("NewValue")) = true then
