...
If len(trim(id)) > 0 then
	...
	if rs3.eof=false or rs3.bof=false then
		strName = " By " & rs3("Firstname") & " " & rs3("Surname")
	end if
	rs3.close
end if
GetUserName = strName
...
dim FieldData(100), SecurityData(100), FormData(100), FieldLength(100), CanEdit(100),FieldType(100),Visibility(100), intFields, StudyID, NewNoteOrSite
...
if rs2.eof=false or rs2.bof=false then
	...
	strLastUpdated = rs2("LastUpdate")
	strLastUpdatedBy = rs2("LastUpdatedBy")
	strDateStamp = rs2("DateStamp")
end if
rs2.close
...
do until rs.eof=true
	...
	Visibility(rs("FieldID")) = rs("Visibility") & ""
	...
	'CODE TO HIDE SPECIFIED FIELDS
	if Visibility(rs("FieldID"))<> "All" then
		if instr(";" & Visibility(rs("FieldID")) & ";", ";" & strDepartment & ";")=0 then
			CanEdit(rs("FieldID")) = "N"
		end if
	end if
...
for Section = 1 to 5
	...
	case 2
		Response.Write "Study Set-Up"
	case 3
		Response.Write "Study Timeline"	
	case 4
		Response.Write "Reporting"
	case 5
	...
	do until rs.eof=true
		...
		if rs("FieldName") = "InLifeCompletionDate" then
			c=c+2
			Response.Write "<td></td><td></td>"	
		end if
		if rs("FieldName") = "ActualExperimentalEndDate" then
			c=c+1
			Response.Write "<td></td><td></td>"	
		end if
		Response.Write "<td valign=top>" 
		if Visibility(rs("FieldID"))="All" or instr(";" & Visibility(rs("FieldID")) & ";", ";" & strDepartment & ";")>0 then
			...
			if rs("FieldType") = "Date" then
				rsTrigger.open "SELECT * FROM Triggers WHERE TriggerField = '" & FieldData(rs("FieldID")) & "' AND StudyID = " & StudyID & " AND IsActive = 1", db
				if rsTrigger.eof=false or rsTrigger.bof=false then
					Response.Write " <font size=1 color=red>(T)</font>"
				end if
				rsTrigger.close
			else
				Response.Write "."
		...
		if Visibility(rs("FieldID"))="All" or instr(";" & Visibility(rs("FieldID")) & ";", ";" & strDepartment & ";")>0 then
			...
			if rs("FormFieldType") = "Combo" then
				if FieldData(rs("FieldID")) = "StudyType" or FieldData(rs("FieldID")) = "TestSubstanceName" or FieldData(rs("FieldID")) = "RegulatoryStatus" or FieldData(rs("FieldID")) = "FinancialClientName2"  or FieldData(rs("FieldID")) = "SEMSTestSubstanceName" then
		...
		Response.Write "</td>"
		if rs("FieldName") = "ProjectManagementCode" or rs("FieldName") = "InLifeCompletionDate" then
			c=c+1
	    end if
...
response.Write "Last Updated at " & FlipDateView(strLastUpdated) & " " & FormatDateTime(strLastUpdated, 3) & GetUserName(strLastUpdatedBy)
response.Write "</td>"
response.Write "<td colspan=2 align=right>Date Stamp : " & FlipDateView(strDateStamp) & " " & FormatDateTime(strDateStamp, 3) & "</td>"
response.Write "</tr>"
