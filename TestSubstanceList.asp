...
<%
	...
	if trim(Request.QueryString("Action"))="D" then
		if Request.QueryString("WebKey") > "" then
			...
			strsql = "DELETE FROM TestSubstances WHERE WebKey = '" & t & "'"
...
%>
<blockquote><table width="100%" cellspacing="0" cellpadding="5"><tr><td valign="top"><h1>Test Substance List</h1>&nbsp;&nbsp;<a href=EditTestSubstanceList.asp>Add Test Substance Name</a></td><td valign=top></td>
<td align="right" valign="top"><form method="post" action="TestSubstanceList.asp"><a href=TestSubstanceList.asp?b1=Search>Clear Search</a>&nbsp;&nbsp;<input type="Text" name="Search" value="<%=OriginalSearch%>">
<%
...
if Request.Form("ClientEdit")="" then
	strsql = "SELECT * FROM TestSubstances WHERE TestSubstanceID > 0 AND"
	if Search > "" then
		s = split(Search, " ")
		'strsql = strsql & " WHERE "
		for i = 0 to ubound(s)
			...
			strsql = strsql & " TestSubstanceName LIKE '" & s(i) & "%' AND"
		next
	...
	strsql = strsql & " ORDER BY TestSubstanceName ASC"
	'Response.Write strsql & "<br>"
	DisplayFields = "TestSubstanceName"
else
	...
	if rs.eof=false or rs.bof=false then
		...
		if cint(Page)> 1 then
			Response.Write "<a href=TestSubstanceList.asp?Page=" & cint(Page)-1 & "&Search=" & Search & "&AllRecords=" & AllRecords & "&ShowInactive=" & ShowInactive & ">< Previous Page</a>&nbsp;"
		else
		...
		for i = startpage to endpage
			if cint(Page) = cint(i) then
				...
				Response.Write "<a href=TestSubstanceList.asp?Page=" & i & "&Search=" & Search & "&AllRecords=" & AllRecords & "&ShowInactive=" & ShowInactive & ">" & i & "</a>&nbsp;"
			end if
				
		next
		if cint(Page)< rs.PageCount then
			Response.Write "&nbsp;<a href=TestSubstanceList.asp?Page=" & cint(Page)+1 & "&Search=" & Search & "&AllRecords=" & AllRecords & "&ShowInactive=" & ShowInactive & "> Next Page ></a>"
		else
		...
		Response.Write "<td><b>Test Substance Name</b></td>"
		...
		do until rs.eof=true or c=pagesize
			...
			rs2.open "SELECT * FROM Studies WHERE SEMSTestSubstanceName = '" & replace(rs("TestSubstanceName") & "","'","''") & "'",db
			if rs2.eof=true and rs2.bof=true then
				Response.Write "<td valign=top><a href=""TestSubstanceList.asp?Action=D&WebKey=" & rs("WebKey") & """>Delete</a></td>"
				Response.Write "<td valign=top><a href=""EditTestSubstanceList.asp?Key=" & rs("WebKey") & """>Edit</a></td>"
			else
				Response.Write "<td valign=top>In Use</td>"
				Response.Write "<td valign=top>In Use</td>"
			end if
			rs2.close
