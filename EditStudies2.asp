...
Response.Write "<form method=""Post"" action=""EditStudies2.asp"">"
...
for Section = 1 to 4
	...
	do until rs.eof=true
		...
		if rs("FormFieldType") = "Combo" then
			if FieldData(rs("FieldID")) = "StudyType" or FieldData(rs("FieldID")) = "TestSubstanceName" then
		...
		if rs("FormFieldType") = "CheckBox" then
			Response.Write CheckBoxHTML("ExcludeFromOverdue", "-1", EditDisabled)
		end if
...
if EditDisabled = "" then
	Response.Write "<href=""#"" id=""addmultisite"" onClick=""toggle_it('trnewmultisite');toggle_it('addmultisite');""><u>Add Site</u></a>"
end if
Response.Write "</td><tr>"
...
rs.open "SELECT * FROM MultiSite WHERE StudyID = " & StudyID
do until rs.eof=true
	Response.write "<tr>"
...
if EditDisabled = "" then
	Response.Write "<href=""#"" id=""addnote"" onClick=""toggle_it('trnewnote');toggle_it('addnote');""><u>Add Note</u></a>"
end if
Response.Write "</td><tr>"
Response.Write "<tr><td colspan=4>"
Response.Write "<table width=""100%"">"
...
do until rs.eof=true
	Response.write "<tr>"
	Response.write "<td width=""75%"">" & rs("Note") & "</td>"
	Response.write "<td>" & rs("CreatedBy") & "</td>"
	Response.write "<td align=right>" & formatdate(rs("DateStamp")) & "</td>"
